...
/** \brief controller_aumentarSueldo llama a la funcion ll_map,para
 * que esta ultima invoque a la funcion aumentaSueldo (aumenta el sueldo empleados
 * que ganan mas de 20.000 en un 15%)
 * luego guarda la linkedList* que devuelve ll_map con los sueldos ya modificados
 * para poder salvar los datos mediante un controller_saveAsText-
 * Borra la lista una vez que los datos estan en un archivo externo.
 * \param pArrayListEmployee LinkedList*
 * \return 1 si el puntero a linkedList recibido es NULL
 *         0 si funciono correctamente
 */
int controller_aumentarSueldo(LinkedList* pArrayListEmployee)
{
	int exito=0;
